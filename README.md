...
agent-browser、browser-use，以及**飞书 / 企业微信命令行工具**。
...
| `wecom-cli` | `npm i -g @wecom/cli` | 企业微信 CLI |
...
skills 包和授权凭证都是运行期内容（且授权需要浏览器 / 扫码交互），刻意不烤进镜像，也不受镜像更新影响。
容器起来后在容器内各执行一次即可：
...
# 企业微信 —— 14 个 wecomcli-* skills
npx skills add WecomTeam/wecom-cli -y -g
wecom-cli auth init --noninteractive  # 展示授权链接 + 二维码，扫码后自动退出（仅一次）
wecom-cli auth show --status          # 需要输出 authorized
```
